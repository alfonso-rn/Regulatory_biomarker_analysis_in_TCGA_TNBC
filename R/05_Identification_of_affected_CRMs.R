# 5. Identification of intragenic CRMs affected by mutations in DEGs ----
#
# Candidate cis-regulatory modules (CRMs) overlapping intronic mutations in
# differentially expressed genes (DEGs) from TNBC samples are identified
# using genomic coordinates from the mutation dataset of TCGA.
#
...
#
# CRMs are prioritized when the mutated DEG, the gene overlapped by the CRM,
# and the predicted CRM target gene correspond to the same gene. This criterion
# identifies intragenic CRMs potentially associated with the regulation of the
...
# 5.5. Identification of intragenic CRMs in mutated DEGs ----
...
intragenicCRMs <- filtCRMs[!is.na(filtCRMs$common_genes), ]

cat("CRMs located in and targeting the same mutated DEG:", nrow(intragenicCRMs))
cat("Mutated DEGs potentially regulated by these CRMs:", 
    paste(unique(intragenicCRMs$common_genes), collapse = "; "))

# Add expression TNBC status level column from DEA
intragenicCRMs$expression_status <- dataDEA$expression_status[match(intragenicCRMs$common_genes, dataDEA$gene_name)]

write.csv(intragenicCRMs, "results/tables/intragenicCRMs.csv", row.names = FALSE)
saveRDS(intragenicCRMs, "data/processed/intragenicCRMs.rds")

# 5.7. TFs associated with identified CRMs ----

CRMs_tfac_list <- vector("list", nrow(intragenicCRMs))

for (i in seq_len(nrow(intragenicCRMs))) {
  
  crm <- intragenicCRMs$crm_name[i]
...
# 6.6. Identification of intragenic CRMs in mutated DEGs ----
